...
class IconView @JvmOverloads constructor(
    ...
    private fun createShadows() {
        ...
        if (bigBlurShadow == null) {
            bigBlurShadow = Bitmap.createBitmap(shadowBounds.width().toInt(), shadowBounds.height().toInt(), ARGB_8888)
        } else {
            bigBlurShadow?.eraseColor(Color.TRANSPARENT)
        }

        if (smallBlurShadow == null) {
            smallBlurShadow = Bitmap.createBitmap(shadowBounds.width().toInt(), shadowBounds.height().toInt(), ARGB_8888)
        } else {
            smallBlurShadow?.eraseColor(Color.TRANSPARENT)
        }

        val canvas = Canvas()
        createShadow(bigBlurShadow, canvas, bigBlurRadius)
        createShadow(smallBlurShadow, canvas, smallBlurRadius)
    }
